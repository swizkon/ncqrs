...
namespace Ncqrs.Eventing.Storage.MSSQL
{
    ...
    public class MultiAppSqlServerEventStore : IEventStore, ISnapshotStore
    {
        ...
        public MultiAppSqlServerEventStore(
            ...
            : this(connectionString, applicationName, null, null)
        {
        ...
        public IEnumerable<Guid> GetAllIdsForType(Type eventProviderType)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                using (var command = new SqlCommand(MultiAppQueries.SelectAllIdsForTypeQuery, connection))
                {
        ...
        public IEnumerable<CommittedEvent> GetEventsAfter(Guid? eventId, int maxCount)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                var query = eventId.HasValue ? MultiAppQueries.SelectEventsAfterQuery : MultiAppQueries.SelectEventsFromBeginningOfTime;
        ...
        public Snapshot GetSnapshot(Guid eventSourceId, long maxVersion)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                using (var command = new SqlCommand(MultiAppQueries.SelectLatestSnapshot, connection))
                {
                    command.Parameters.AddWithValue("@ApplicationName", _applicationName);
        ...
        public CommittedEventStream ReadFrom(Guid id, long minVersion, long maxVersion)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                using (var command = new SqlCommand(MultiAppQueries.SelectAllEventsQuery, connection))
                {
        ...
        public void RemoveUnusedProviders()
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                using (var command = new SqlCommand(MultiAppQueries.DeleteUnusedProviders, connection))
                {
                    connection.Open();
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        using (var dataStream = new MemoryStream())
                        {
                            ...
                            using (var command = new SqlCommand(MultiAppQueries.InsertSnapshot, transaction.Connection))
                            {
        ...
        private static void AddEventSource(String applicationName, Guid eventSourceId, Type eventSourceType, long initialVersion, SqlTransaction transaction)
        {
            using (var command = new SqlCommand(MultiAppQueries.InsertNewProviderQuery, transaction.Connection))
            {
                command.Transaction = transaction;
        ...
        private static int? GetVersion(String applicationName, Guid eventSourceId, SqlTransaction transaction)
        {
            using (var command = new SqlCommand(MultiAppQueries.SelectVersionQuery, transaction.Connection))
            {
                command.Transaction = transaction;
                command.Parameters.AddWithValue("ApplicationName", applicationName);
                command.Parameters.AddWithValue("id", eventSourceId);
        ...
        private static bool UpdateEventSourceVersion(String applicationName, Guid eventSourceId, long newVersion, long initialVersion, SqlTransaction transaction)
        {
            using (var command = new SqlCommand(MultiAppQueries.UpdateEventSourceVersionQuery, transaction.Connection))
            {
                command.Transaction = transaction;
                command.Parameters.AddWithValue("ApplicationName", applicationName);
                command.Parameters.AddWithValue("Id", eventSourceId);
        ...
        private void SaveEvent(UncommittedEvent uncommittedEvent, SqlTransaction transaction)
        {
            ...
            using (var command = new SqlCommand(MultiAppQueries.InsertNewEventQuery, transaction.Connection))
            {
                command.Transaction = transaction;
                command.Parameters.AddWithValue("ApplicationName", this._applicationName);
        ...
        private void StoreEventsFromSource(Guid eventSourceId, long eventSourceVersion, IEnumerable<UncommittedEvent> events, SqlTransaction transaction)
        {
            ...
            long? currentVersion = GetVersion(this._applicationName, eventSourceId, transaction);
            long initialVersion = events.First().InitialVersionOfEventSource;

            // Create new event provider when it is not found.)
            if (currentVersion == null)
            {
                AddEventSource(this._applicationName, eventSourceId, typeof(object), eventSourceVersion, transaction);
            }
            ...
            {
                UpdateTheVersionOfTheEventSource(this._applicationName, eventSourceId, eventSourceVersion, transaction, initialVersion);
            }
        ...
        private static void UpdateTheVersionOfTheEventSource(String applicationName, Guid eventSourceId, long eventSourceVersion, SqlTransaction transaction, long initialVersion)
        {
            var updated = UpdateEventSourceVersion(applicationName, eventSourceId, eventSourceVersion, initialVersion, transaction);
